...
public class RxLifecycleAndroid {
    ...
    private static final Function<FragmentEvent, FragmentEvent> FRAGMENT_LIFECYCLE =
            new Function<FragmentEvent, FragmentEvent>() {
                @Override
                public FragmentEvent apply(
                        FragmentEvent lastEvent) throws Exception {
                    switch (lastEvent) {
                        case ATTACH:
     ...
     *                  * @return a reusable {@link io.reactivex.ObservableTransformer} that unsubscribes the source during the Activity lifecycle
     */
    @NonNull
    @CheckResult
     ...
     * @return a reusable {@link io.reactivex.ObservableTransformer} that unsubscribes the source during the View lifecycle
     */
    @NonNull
    @CheckResult
    public static <T> LifecycleTransformer<T> bindView(@NonNull View view) {
        ...
        return bind(RxJavaInterop.toV2Observable(RxView.detaches(view)));
    }
